...
import { solveRoom, type SolveStep } from "../src/world/routeSolver";
...
type GhostStep = { a: "kill" | "warp" | "chain"; f: number[]; t?: number[] };
type Certificate = { hash: string; solved: boolean; warps: number; ms: number; reason?: string; route?: GhostStep[] };
type Entry = { key: string; room: RoomSpec; exact: boolean; blocking: boolean; ghost?: boolean };
...
const entries: Entry[] = [
  ...
  ...buildTimeTrialSuite().map((room) => ({ key: `time-trial:${room.id}`, room, exact: false, blocking: true, ghost: true })),
...
for (const entry of entries) {
  ...
  // Time Trial certificates also carry the proven route, used as the in-game route ghost.
  const needsRoute = entry.ghost && cached?.solved && !cached.route;
  if (cached && cached.hash === hash && (cached.solved || !entry.blocking) && !needsRoute) {
  ...
  const certificate: Certificate = {
    hash, solved: result.solved, warps: result.warps, ms: Date.now() - started, reason: result.reason,
    ...(entry.ghost && result.solved ? { route: result.steps.map(ghostStep) } : {})
  };
...
// Route ghosts ship with the game: room id -> proven route, regenerated whenever geometry changes.
const ghosts: Record<string, GhostStep[]> = {};
for (const entry of entries) {
  const route = next[entry.key]?.route;
  if (entry.ghost && route) ghosts[entry.room.id] = route;
}
writeFileSync(
  new URL("../src/world/generated/routeGhosts.ts", import.meta.url),
  `// Generated by scripts/route-certify.ts. Do not edit.\nexport type GhostStep = { a: "kill" | "warp" | "chain"; f: number[]; t?: number[] };\nexport const ROUTE_GHOSTS: Record<string, GhostStep[]> = ${JSON.stringify(ghosts)};\n`
);
...
function ghostStep(step: SolveStep): GhostStep {
  const r = (v: readonly number[]) => v.map((n) => Math.round(n * 100) / 100);
  return { a: step.action, f: r(step.from), ...(step.to ? { t: r(step.to) } : {}) };
}
