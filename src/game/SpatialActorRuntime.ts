...
/**
 * Spheres remain Traversal's movement actors. Cubes, Diamonds and Prisms never
 * write player movement; they alter the spatial problem around the spheres.
 */
export function installSpatialActorRuntime(game: object): void {
  ...
  state.shoot = () => {
    ...
    const liveMeshes = state.enemies.filter((enemy) => enemy.alive).map((enemy) => enemy.mesh);
    const deviceMeshes = devices.map((device) => device.mesh);
    const hit = state.raycaster.intersectObjects(
      [...deviceMeshes, ...liveMeshes, ...state.platformMeshes],
      false
    )[0];

    const direction = new THREE.Vector3(0, 0, -1).applyQuaternion(state.camera.quaternion);
    const endPoint = hit?.point.clone() ?? state.camera.position.clone().add(direction.multiplyScalar(120));
  ...
  state.updateTargetReticle = () => {
    ...
    const enemy = hit ? state.enemies.find((candidate) => candidate.mesh === hit.object) : undefined;
...
function bindPlatforms(root: THREE.Group, specs: PlatformSpec[], meshes: THREE.Mesh[]): PlatformBinding[] {
  return specs.map((spec, index) => {
    ...
    const edge = root.children.find((child) =>
      child instanceof THREE.LineSegments && child.position.distanceTo(mesh.position) < 0.001
...
function deviceMessage(device: ActiveDevice): string {
  const value = device.active ? "ON" : "OFF";
  if (device.spec.kind === "cube") return `CUBE // STATE ${value}`;
  if (device.spec.kind === "diamond") return `DIAMOND // MOTION ${value}`;
  return `PRISM // ENERGY ROUTE ${value}`;
}
