...
const field09: RoomSpec = {
  id: "sector-09-crossings", title: "CROSSINGS", lesson: "Destinations are trajectories, not points.",
  grammar: ["stop-short", "moving-endpoint", "route-fork"], spawn: [0, 2.2, 10], goal: [0, 1.1, -76], requiredKills: 4,
  platforms: [...start(), { center: [-9, 0, -17], size: [8, 1, 8] }, { center: [10, 2, -36], size: [8, 1, 8] }, { center: [-7, 1, -55], size: [8, 1, 8] }, finish(0, -76)],
  enemies: [
...
const field10: RoomSpec = {
  id: "sector-10-sightlines", title: "SIGHTLINES", lesson: "A destination matters because of the angle it gives you.",
  grammar: ["origin-matters", "reorientation", "airborne-chain"], spawn: [-8, 2.2, 9], goal: [8, 1.1, -72], requiredKills: 4,
  platforms: [...start(-8), { center: [0, 4, -14], size: [1, 8, 18] }, { center: [10, 2, -18], size: [7, 1, 7] }, { center: [-8, 1, -39], size: [7, 1, 7] }, { center: [4, 4, -50], size: [1, 8, 18] }, finish(8, -72)],
  enemies: [
...
const field12: RoomSpec = {
  id: "sector-12-gates", title: "GATES", lesson: "Read physical and timed openings as one shared safe region.",
  ...
  hazards: [{ id: "a2-12-pulse", kind: "sightline-gate", center: [0, 4, -55], size: [18, 8, 1], cycle: { period: 3.2, openFor: 1.35 } }]
};
...
const field14: RoomSpec = {
  id: "sector-14-refraction", title: "REFRACTION", lesson: "The safe line and the fast line are not always the same line.",
  ...
  ], hazards: [{ id: "a2-14-sweep", kind: "sweep", center: [0, 4, -49], size: [.6, 8, 22], drift: { axis: "x", amplitude: 16, speed: .45 } }]
};

const field15: RoomSpec = {
  id: "sector-15-machinery", title: "MACHINERY", lesson: "Treat the construct as one machine. Position, timing and sightline are coupled.",
  grammar: ["moving-endpoint", "origin-matters", "reorientation", "route-fork"], spawn: [0, 2.2, 10], goal: [0, 1.1, -84], requiredKills: 5,
  platforms: [...start(), { center: [-10, 1, -20], size: [8, 1, 8] }, { center: [10, 4, -40], size: [8, 1, 8] }, { center: [-10, 2, -61], size: [8, 1, 8] }, finish(0, -84)],
  enemies: [
...
const field16: RoomSpec = {
  id: "sector-16-composition", title: "COMPOSITION", lesson: "Act II exam. Read the whole sentence before you write the first vector.",
  grammar: ["stop-short", "airborne-chain", "moving-endpoint", "reorientation"], spawn: [0, 2.2, 11], goal: [0, 1.1, -104], requiredKills: 6,
  platforms: [...start(), { center: [-11, 2, -25], size: [7, 1, 7] }, { center: [11, 4, -48], size: [7, 1, 7] }, { center: [-8, 2, -72], size: [7, 1, 7] }, finish(0, -104)],
  enemies: [
    ...
    { id: "a2-16-gate", kind: "sightline-gate", center: [0, 5, -88], size: [22, 10, 1], cycle: { period: 3, openFor: 1.25, phase: .4 } }
  ]
};
...
export const ACT_II_MAPS: CampaignMapDefinition[] = [
  ["map-09", "SECTOR 09 // CROSSINGS", "Destinations become trajectories.", ["Stop Short", "Moving Endpoint"], field09],
  ["map-10", "SECTOR 10 // SIGHTLINES", "Position creates the shot.", ["Origin", "Reorientation", "Occlusion"], field10],
  ["map-11", "SECTOR 11 // RELAY", "Aerial acquisition becomes continuous language.", ["Aerial Chain", "Moving Endpoint"], field11],
  ["map-12", "SECTOR 12 // GATES", "Physical and timed openings overlap.", ["Origin", "Timing", "Stop Short"], field12],
  ["map-13", "SECTOR 13 // FORK", "Useful is not the same as necessary.", ["Route Fork", "Efficiency"], field13],
  ["map-14", "SECTOR 14 // REFRACTION", "Safe lines, fast lines and hidden lines diverge.", ["Route Choice", "Reorientation", "Hazards"], field14],
  ["map-15", "SECTOR 15 // MACHINERY", "The room reads as one coupled machine.", ["Moving Endpoint", "Origin", "Timing"], field15],
  ["map-16", "SECTOR 16 // COMPOSITION", "Act II examination.", ["Synthesis", "Four-System Composition"], field16]
